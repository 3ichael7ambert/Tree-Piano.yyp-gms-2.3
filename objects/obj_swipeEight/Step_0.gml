/// @description The good stuffs (swipe handling)
/// @param swipe handling

//This will set the init variables to be the first position on the screen that is touched.
//In addition, set the isTouching variable to one, indicating that the screen is being touched.
if (mouse_check_button_pressed(mb_left))
{
    initX = mouse_x;
    initY = mouse_y;
    isTouching = 1;
}

//Once the user stops touching the screen, reset the variables.
if (mouse_check_button_released(mb_left))
{
    isTouching = 0;
    //This will set the objects direction to be equal to the swiped direction.
    direction = point_direction(initX, initY, mouse_x, mouse_y);
}



///Direction handling

//The following will adjust the object's direction to be exactly one of the eight standard top down directions.
//(N, NW, W, SW, S, SE, E, NE)
//To make the object move in the exact direction the user swiped, just get rid of this code block.

//Right
if ((direction >= 0) && (direction <= 22.5)) || ((direction <= 360) && (direction >= 337.5))
{
    direction = 0;
}

//Up/Right
if ((direction > 22.5) && (direction <=67.5))
{
    direction = 45;
}

//Up
if ((direction > 67.5) && (direction <=112.5))
{
    direction = 90;
}

//Left/Up
if ((direction > 112.5) && (direction <=157.5))
{
    direction = 135;
}

//Left
if ((direction > 157.5) && (direction <=202.5))
{
    direction = 180;
}

//Left/Down
if ((direction > 202.5) && (direction <=247.5))
{
    direction = 225;
}

//Down
if ((direction > 247.5) && (direction <=292.5))
{
    direction = 270;
}

//Down/Right
if ((direction > 292.5) && (direction <337.5))
{
    direction = 315;
}

///Wrap around the screen

//This code makes the object pop to the other side of the screen when nearing the edge of the room
//like pacman!
if (x > room_width) then x = 0;
if (x < 0) then x = room_width;
if (y > room_height) then y = 0;
if (y < 0) then y = room_height;

